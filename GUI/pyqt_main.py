...
class Window(QMainWindow):
    ...
    def __init__(self):
        ...
        # add the current prediction to phrases
    ...
    def update_image(self, cv_img):
        qt_img = self.convert_cv_qt(cv2.rectangle(
            cv_img, (300, 300), (100, 100), (0, 255, 0), 0))
...
if __name__ == "__main__":
    ...
    window.arabicNotation.setStyleSheet(
        "text-align: center;font-size: 30px;border: 2px solide;")
    window.textLabel.setStyleSheet(
        "text-align: center;font-size: 30px;border: 2px solide;")
    window.title.setStyleSheet(
        "text-align: center;font-size: 40px; color:#000000")
    window.image_label.setStyleSheet(
        "background:darkGray;border-radius: 20px;")
    window.show()
